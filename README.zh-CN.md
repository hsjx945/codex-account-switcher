...
| **邮箱与昵称** | 默认显示完整登录邮箱，可设置昵称，并选择显示邮箱、昵称或两者。 |
| **套餐与北京时间** | 使用官方账号信息识别 Plus、Pro 与 Pro Lite，所有用量条等宽对齐，重置时间统一为中文北京时间；仅当服务端实际返回精确 300 分钟窗口时才显示 5 小时额度。 |
| **Token 与模型统计** | 单账号显示今天、近 7 天、近 30 天服务端 Token 总量；模型明细按这台 Mac 上现存的全部账号会话单独统计。 |
| **额度恢复提醒** | 可选开启；官方 5 小时窗口确认推进后发送提醒。提醒内可切换账号，检测到运行任务或状态未知时必须再次确认。 |
| **实验性预热** | 默认关闭；显式开启后按设定时间用可用小模型发送一次最小真实请求，并跳过仍活跃的 5 小时窗口。 |
...
- 已保存的 profile 仍使用文件型凭证。每次切换会生成一个短期加密回滚副本；加密密钥保存在 macOS 钥匙串，验证完成后删除副本。
- 产品不使用自己的账号代理、流量转发或云端账号服务。
- 每个账号都由用户选择并确认；应用不会自动轮换账号。
- 本项目是独立开源软件，与 OpenAI 没有关联，也未获得 OpenAI 背书。
- 当前账号始终置顶，并使用低饱和背景和明确的“当前”标签与其他账号区分。
- 额度恢复提醒默认关闭；通知动作只按本地 profile UUID 路由，任务运行中或状态未知时绝不会静默切换。
- 预热是会消耗少量用量的真实请求，只是社区验证过的实验性做法；本项目不承诺 OpenAI 一定据此启动或刷新 5 小时窗口。
- 获取最新数据时，界面会继续显示已保存的 5 小时和每周用量；5 小时用量仅在开启且服务端提供精确 300 分钟窗口时显示。
- 额度刷新、登录、删除账号和切换共用串行执行门，避免并发改写同一个 profile。
- 活动凭证变化前先写入持久化切换日志。切换要么得到已验证的目标账号，要么恢复加密保存的原始凭证并验证原账号。
- 应用启动时会恢复中断事务：已提交的目标账号保持不变；更早阶段回滚，并恢复原先的 Desktop 运行状态。
- 自动轮换账号、策略化路由、远程控制、遥测和云端凭证存储仍不在产品范围内。
